...
if N > 0
    ...
    fprintf(f,'%s\n', datestr(now));
    fprintf(f, '%d\n', N); % Save N at the 2nd top of progress.txt
    fclose(f);
    
    if nargout == 0
        disp(['      /     : 00.00[>', repmat(' ', 1, w), ']']);
    end
elseif N == 0
    ...
    if nargout == 0
        disp([repmat(char(8), 1, (w+24)), char(10), '      /     : 00.00[', repmat('=', 1, w+1), ']']);
    end
else
    ...
    finished = length(progress) - 1;
    total = progress(1);
    percent = finished/total*100;
    time_remain = etime(datevec(now), datevec(t_start))/percent*(100-percent);
    time_remain = time_remain/3600;
    
    if nargout == 0
        perc = sprintf('%5.0f /%5.0f: %5.2f', finished, total, time_remain); % 4 characters wide, percentage
        disp([repmat(char(8), 1, (w+24)), char(10), perc, '[', repmat('=', 1, round(percent*w/100)), '>', repmat(' ', 1, w - round(percent*w/100)), ']']);
    end
end
